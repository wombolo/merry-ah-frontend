...
export class CreateArt extends Component {
  ...
  eventHandlers = () => {
    ...
    return {
      addedfile: (file) => {
        this.setState({ numberOfFiles: numberOfFiles += 1 });
        if (numberOfFiles <= 7) {
  ...
  render() {
    ...
    const djsConfig = {
      ...
      dictDefaultMessage:
        '<i class="fa fa-3x fa-cloud-upload"> </i> <br/>Drag an image here to upload, or click to select multiple',
      maxFilesize: 50, // MB
      addRemoveLinks: true,
    };
      ...
      <div className="container mt-5 pt-5">
        <form onSubmit={this.handleSubmit}>
          <div className='col-md-6 offset-md-3'>
            ...
            <div className="form-group">
              <label>Price</label>
              <input type="number" step={0.01} className="form-control col-8" defaultValue={0.00}
                     onChange={this.handleChange} name='price'
                     placeholder="Enter Price"/>
                     <small className='text-info'>Price set to 0 means item is not for sale</small>
            </div>
